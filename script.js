const SITE_CONFIG = {
  ...
  waitTime: 25, // tempo médio de retirada em minutos — troque quando quiser
  hours: [
...
function updateWaitTimeBadge() {
  const badges = document.querySelectorAll(".wait-time-badge");
  ...
  badges.forEach((badge) => {
    ...
    badge.hidden = false;
    badge.querySelector(".wait-time-value").textContent = SITE_CONFIG.waitTime;
  });
}
...
function init() {
  ...
  updateWhatsAppFloat();
  updateWaitTimeBadge();
  setInterval(() => { updateWhatsAppFloat(); updateWaitTimeBadge(); }, 60000);
